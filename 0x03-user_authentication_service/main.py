...
def register_user(email: str, password: str) -> None:
    ...
    res = requests.post(f'{URL}/users', data=user)
...
def log_in_wrong_password(email: str, password: str) -> None:
    ...
    user = {"email": email, "password": password}
    res = requests.post(f'{URL}/sessions', data=user)
    print(res.status_code, res.text)  # Log the response for debugging
...
def profile_logged(session_id: str) -> None:
    ...
    res = requests.get(f'{URL}/profile', headers=headers)
    print(res.status_code, res.text)  # Log the response for debugging
    assert res.status_code == 200
...
def log_out(session_id: str) -> None:
    ...
    headers = {"Authorization": f"Bearer {session_id}"}
    res = requests.delete(f'{URL}/sessions', headers=headers)
    print(res.status_code, res.text)  # Log the response for debugging
